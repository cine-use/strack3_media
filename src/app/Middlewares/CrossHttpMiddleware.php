...
class CrossHttpMiddleware extends HttpMiddleware
{
    ...
    {
        parent::__construct();
        if (CrossHttpMiddleware::$cache404 == null) {
            $template = get_instance()->loader->view('server::404');
            CrossHttpMiddleware::$cache404 = $template;
        }
    }

    /**
     * @throws \Server\CoreBase\SwooleInterruptException
     */
    public function before_handle()
    {
        ...
        if ($path == '/404') {
            ...
            $this->response->end(CrossHttpMiddleware::$cache404);
            $this->interrupt();
        }
